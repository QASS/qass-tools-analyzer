...
class InputObserver:
    ...
    def __init__(self, inputs: List[Tuple[int, int, bool, int]], callback):
        """
        ...
        """
        # callback is a function with the signature ((byte, bit, state, delay), start_time, end_time)
        self._inputs = inputs
        self._stream = None
    ...
    def process_start(self, stream: Buffer_Py_IF) -> None:
        """
    ...
    def tick(self, current_time: int, ignore_getIO_exception: bool = False) -> None:
        """
        ...
        curr_spec = int(current_time // spec_duration)
    ...
    def process_end(self, current_time) -> None:
        """
        ...
        """
        self.tick(current_time, ignore_getIO_exception=True)
        
        for cfg, start, end in self._finished_ranges:
            self._callback(cfg, start, current_time)
        
        for idx, (byte, bit, state, delay) in enumerate(self._inputs):
            if self._current_input_times[idx] is not None:
                self._callback((byte, bit, state, delay), self._current_input_times[idx], current_time)
